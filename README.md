...
`~/ros2_ws/src/` 底下共 **9 個 ROS 2 package**：1 個機器人描述包
（`hiwin_dual_arm_description`）、1 個 MoveIt2 設定總包（`hiwin_dual_arm`）、6 個可插拔的
雙臂避障「規劃器」方案，以及 1 個跨機/跨 domain 用的橋接包（`dual_arm_domain_bridge`，
細節見該 package 自己的 `README.md`）。各規劃器的演算法推導、參數對照表請見個規劃器的
...
### 3.1 第一次拿到這個 workspace（依序執行）

```bash
# 步驟 1：載入 ROS 2 Humble 環境
source /opt/ros/humble/setup.bash

# 步驟 2：進入 workspace 根目錄
cd ~/ros2_ws

# 步驟 3：初始化 rosdep（整台機器只需做一次；之前已 init 過會報錯，忽略即可）
sudo rosdep init
rosdep update

# 步驟 4：依 package.xml 自動安裝所有依賴（見 3.0 表格）
rosdep install --from-paths src --ignore-src -r -y

# 步驟 5：編譯整個 workspace（第一次跑，或改過 src/*.cpp 才需要）
colcon build --symlink-install

# 步驟 6：載入這個 workspace 的環境（每開一個新終端機都要重做）
source install/setup.bash

# 步驟 7：一鍵啟動：TF + robot_state_publisher + move_group + RViz
...
> `colcon build`。之後每次開新終端機，只需重做步驟 1、2、6、7（跳過步驟 3~5 的建置），
> 除非改了 C++ 原始碼。
...
- **`colcon build` 報 `Eigen3` / `eigen3_cmake_module` 找不到**：代表依賴沒裝齊，回到
  [3.1](#31-第一次拿到這個-workspace依序執行) 跑一次 `rosdep install`；或手動
